...
SPIKE_MUTATIONS = {
    ...
    "Omicron": [67, 69, 70, 95, 142, 143, 144, 211, 212, 339, 371, 373, 375,
                417, 440, 446, 477, 478, 484, 493, 496, 498, 501, 505, 547,
                614, 655, 679, 681, 764, 796, 856, 954, 969, 981],
    "Recent": [67, 69, 70, 95, 142, 143, 144, 211, 339, 371, 373, 375,
               417, 440, 446, 452, 478, 484, 486, 490, 493, 498, 501, 505,
               614, 655, 679, 681, 764, 796, 954, 969, 981, 1020],
}
...
def plot_gc_content_distribution(df, output_path=None, show=True):
    ...
    for variant in variants:
        ...
        ax2.hist(variant_data, bins=15, alpha=0.5, label=variant,
                 color=VARIANTS[variant]["color"], density=True)
...
def plot_sequence_length_distribution(df, output_path=None, show=True):
    ...
    ax2.axvline(x=df['length'].mean(), color='green', linestyle='-', linewidth=2,
...
def plot_phylogenetic_tree(df, output_path=None, show=True):
    ...
    if HAS_BIOPYTHON:
        ...
        ax.set_title('Phylogenetic Tree of SARS-CoV-2 Variants\n(Based on Spike Protein Mutations)',
        ...
        for variant, info in tree_structure.items():
            if info['parent']:
                ...
                ax.plot([parent_info['x'], info['x']-0.3, info['x']],
        ...
        for variant, info in tree_structure.items():
            ...
            ax.annotate(variant, (info['x'], info['y']), fontsize=11, fontweight='bold',
...
def plot_mutation_heatmap(output_path=None, show=True):
    ...
    ax.set_xticklabels([all_positions[i] for i in range(0, len(all_positions), step)],
                       rotation=90, fontsize=9)

    ax.set_xlabel('Spike Protein Position', fontsize=12)
    ax.set_ylabel('Variant', fontsize=12)
    ax.set_title('Spike Protein Mutation Heatmap\n(Key mutations defining each variant)',
                fontsize=14, fontweight='bold')

    # Add mutation count annotations on the right
    for i, variant in enumerate(variants):
        count = len(SPIKE_MUTATIONS[variant])
        ax.annotate(f'{count} mutations', xy=(len(all_positions) + 0.5, i),
                   fontsize=10, va='center', color=VARIANTS[variant]['color'], fontweight='bold')
