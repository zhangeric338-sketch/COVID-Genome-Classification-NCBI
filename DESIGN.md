...
## 9. Preconditions (RESOLVED)

These were blocking for the **build**; all are now resolved.

- **P1 — Branch base. ✅ RESOLVED.** `fix/download-issues` was reconciled into `main` (PR #11):
  `main` now carries the BioPython `TextIOWrapper` Windows loader fix and the validated
  986-genome dataset (791 train / 195 test). CI is green (lint + import checks across Python
  3.10–3.12). `feature/degradation-robustness` is rebased onto the reconciled `main`. The
  reconciliation sidelined `main`'s PR #9 download engine in favor of `fix`'s validated pipeline
  (recoverable from history if ever preferred).
- **P2 — Spike compatibility. ✅ RESOLVED.** Reconciled `main`'s `load_genome_dataset(data_dir)`
  takes no `k` argument (the parameter was dead). The spike and
  `tools/robustness_experiment.py` were updated to call `load_genome_dataset(data_dir)`.
- **P3 — Dataset in git. ✅ RESOLVED (kept in git).** The committed genomes are ~10 KB each
  (~20 MB total), small enough that keeping them in version control *aids* exact reproducibility
  rather than bloating the repo. No external storage needed.

---

## 10. Implementation Plan (incremental — do NOT build the whole matrix at once)

1. ~~**Resolve §9 preconditions** (branch base + spike compatibility).~~ ✅ **Done** — `main`
   reconciled (PR #11), feature branch rebased, spike/experiment calls fixed.
